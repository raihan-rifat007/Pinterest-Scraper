    ...
    <div class="stats-grid">
      <div class="stat-card stat-total">
        ...
        <div class="stat-data">
          ...
          <span class="stat-lbl">Total Pins</span>
        </div>
      </div>
      <div class="stat-card stat-downloaded">
        ...
        <div class="stat-data">
          ...
          <span class="stat-lbl">Downloaded</span>
        </div>
      </div>
      <div class="stat-card stat-existing">
        ...
        <div class="stat-data">
          ...
          <span class="stat-lbl">Saved on disk</span>
        </div>
      </div>
  `;
    ...
    html += `
      <div class="stat-card">
        ...
        <div class="stat-data">
          ...
          <span class="stat-lbl">Too small</span>
        </div>
      </div>`;
  }
  ...
  pins.slice(start).forEach((pin, k) => {
    ...
    card.innerHTML = `
      ...
      <div class="pin-overlay">
        <button class="pin-save"><i class="bi bi-download"></i> Save</button>
        <div class="pin-meta">
...
function setActiveTab(view) {
  document.querySelectorAll('.segmented .tab').forEach(t => {
...
function moveIndicator() {
  const active = $$('.segmented .tab.active');
...
function showPinAt(index) {
  ...
  if (isVideo && pmVideo) {
    ...
    if (pmImg) {
      pmImg.classList.remove('hidden', 'zoomed');
  ...
  if (sizeVal) {
    sizeVal.textContent = (pin.width && pin.height) ? `${pin.width} × ${pin.height}` : '—';
  }
...
function renderChart() {
  ...
  window._chart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        x: {
          ticks: { color: dark ? '#8E8E99' : '#5f5f5f' },
          grid: { color: dark ? '#202026' : '#eee' },
        },
        y: {
          ticks: { color: dark ? '#8E8E99' : '#5f5f5f' },
          grid: { display: false },
...
const commands = [
  { id: 'focus-search', icon: 'bi-search', label: 'Search pins', sub: 'Start a new scrape',
    action: () => { closeCP(); openSearchFromPalette(); } },
  ...
  { id: 'open-settings', icon: 'bi-sliders2', label: 'Open Settings', sub: 'Configure options',
    action: () => { closeCP(); $('settings-btn').click(); } },
  { id: 'download-zip', icon: 'bi-file-zip', label: 'Download ZIP', sub: 'Images as archive',
    action: () => {
      closeCP();
      const a = $('exp-zip');
      if (a && !a.classList.contains('hidden')) a.click();
      else window.showToast('No ZIP available yet', 'info');
    } },
  { id: 'export-xlsx', icon: 'bi-file-spreadsheet', label: 'Export XLSX', sub: 'Metadata spreadsheet',
    action: () => {
      closeCP();
      const a = $('exp-xlsx');
      if (a && !a.classList.contains('hidden')) a.click();
      else window.showToast('No XLSX available yet', 'info');
    } },
];
...
function openSearchFromPalette() {
...
function initDensity() {
  const densityBtns = $$('#density-toggle button');
  densityBtns.forEach(b => {
    b.addEventListener('click', () => {
      ...
      localStorage.setItem('ps_density', b.dataset.density);
    });
  });
  const savedDensity = localStorage.getItem('ps_density') || 'default';
...
function initHeroSuggestions() {
  const heroSugs = [
    'dark academia', 'minimal wallpaper', 'aesthetic room',
    'taylor swift', 'interior design', 'tattoo ideas',
  ];
  const container = $('hero-suggestions');
  if (!container) return;
  container.innerHTML = heroSugs.map(s =>
    `<button class="suggestion-pill" data-q="${escapeHtml(s)}">
      <i class="bi bi-search"></i> ${escapeHtml(s)}
    </button>`
  ).join('');
  container.addEventListener('click', (e) => {
    const b = e.target.closest('.suggestion-pill');
    if (!b) return;
...
function initCommandPalette() {
  ...
  if (searchTrigger) searchTrigger.addEventListener('click', openCP);
  if (cp) {
    const backdrop = cp.querySelector('.cp-backdrop');
    if (backdrop) backdrop.addEventListener('click', closeCP);
  }
  if (cpInput) {
        ...
        return `<li role="option" class="sug-user">
          ...
          <span class="sug-texts">
            <span class="sug-name">${highlight(q, s.text)}
              ${s.verified ? `<span class="sug-verified" title="Verified"><i class="bi bi-patch-check-fill"></i></span>` : ''}
            </span>
...
function wireEvents() {
  ...
  if (shortcutsBtn) {
    shortcutsBtn.addEventListener('click', () => {
      window.showToast(
        '⌘K to search · / to focus · ESC to close · ← → to navigate pins',
        'info',
        6000
      );
    });
